...
def get_cmdline_arguements():
    ...
    parser.add_argument("--revert", help="Revert the Configuration",dest='revert',default=False,required=False,action='store_true')
...
def configure_sequence(con,conf_details,revert=False):
    ...
    for detail in conf_details:
        ...
        if not revert:
            ...
            # UnBinding
            command = 'config terminal \n' + \
                      'no arp vrf BACKBONE_VN {} {} ARPA \n'.format(ip_address, mac) + \
                      'no device-tracking binding vlan {} {} interface {} {} \n'.format(vlan_id, ip_address, interface,mac) + \
                      'end\n'
            execute_command(con, command)
...
def apply_configuration(switch_data,conf_data,revert=False):
            ...
            for conf in conf_data.keys():
                if switch == conf:
                    ...
                    try:
                        ...
                        configure_sequence(con,conf_details,revert)
...
if __name__=="__main__":
    ...
    apply_configuration(switch_data, conf_data,args.revert)
